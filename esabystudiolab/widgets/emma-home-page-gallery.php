	...
	protected function render()
	{
		...
		if ($pods->total() > 0) {
			...
			while ($pods->fetch()) {
				...
				// Determine the meta key based on type
				if ($type === 0) {
					$object_id = get_post_meta($post_id, 'product', true);
				} elseif ($type === 1) {
					$object_id = get_post_meta($post_id, 'architecture', true);
				...
				// Debugging: Log object_id if it's empty
				if (empty($object_id)) {

					error_log("Missing object_id for post ID $post_id and type $type");
					// continue;
				}

				// Generate the link
				if ($lang == 'fr') {
					$link = $this->getTransation((int)$object_id, $type);
				} else {
					$link = ($type === 2) ? get_term_link((int)$object_id) : get_permalink((int)$object_id);
				}
		...
		<style type="text/css">
			...
			html {
				overflow-y: scroll;
				/* Ensures scrolling remains functional */
				scrollbar-width: none;
				/* Firefox */
