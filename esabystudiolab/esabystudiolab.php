...
function handle_custom_query_var($query_args, $query_vars)
{
    if (!empty($query_vars['sorte'])) {
        ...
        // Ensure all products are fetched without pagination
        $query_args['posts_per_page'] = -1;
        if (isset($query_args['tax_query'])) {
            foreach ($query_args['tax_query'] as $key => $query) {
                if ($query['taxonomy'] === 'product_type') {
...
function esa_register_product_tags_taxonomy() {
    $labels = array(
        'name'              => _x('ESA Produit balises', 'taxonomy general name'),
        'singular_name'     => _x('ESA Produit Étiqueter', 'taxonomy singular name'),
        'search_items'      => __('Étiquettes de produits de recherche ESA'),
        'all_items'         => __('Toutes les balises de produits'),
        'edit_item'         => __("Modifier l'étiquette du produit"),
        'update_item'       => __("Mettre à jour l'étiquette du produit"),
        'add_new_item'      => __('Ajouter une nouvelle balise de produit'),
        'new_item_name'     => __('Nouveau nom de balise de produit ESA'),
        'menu_name'         => __('ESA Produit balises')
    );
    ...
    $args = array(
        ...
        'query_var'         => true,
        'rewrite'           => array('slug' => 'esa-product-tag'),
        'show_in_rest'      => true,
        'rest_base'         => 'esa-product-tags',
        'rest_controller_class' => 'WP_REST_Terms_Controller',
    );

    register_taxonomy('esa_product_tags', array('product'), $args);
}
add_action('init', 'esa_register_product_tags_taxonomy');
