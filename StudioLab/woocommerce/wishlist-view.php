...
<table
	...
	<tr>
		...
		<th class="product-name collection">
		<span class="nobr">
		<?php
		/**
				 ...
				 * @return string
				 */
				echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Collection', 'yith-woocommerce-wishlist' ), $wishlist ) );
		?>
		</span></th>
		...
		<th class="product-name dimension">
			<span class="nobr">
				<?php echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Dimension', 'yith-woocommerce-wishlist' ), $wishlist ) ); ?>
			</span>
		</th>
		<th class="product-name download">
			<span class="nobr">
				<?php
				echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', pll__( 'Télécharger' ), $wishlist ) );
				 ?>
			</span>
		</th>
		  <th class="product-name price-request">
      <span class="nobr">
	  <?php
     		echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', pll__( 'Demande de prix' ), $wishlist ) );
        ?>
      </span>
    </th>
		...
		foreach ( $wishlist_items as $item ) :
			...
			if ( $product && $product->exists() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					// 	// Get Categories
						...
						if (!empty($terms) && !is_wp_error($terms)) {
							...
							foreach ($terms as $term) {
								$category_links[] = '<a href="' . esc_url(get_term_link($term)) . '" rel="tag">' . esc_html($term->name) . '</a>';
							}
					...
					<td class="product-dimention">
					<?php
					do_action( 'yith_wcwl_table_before_product_name', $item, $wishlist );
					$product_id = $item['product_id'];
					$dimention_in = pods_field('product', $product_id, 'dimension-inch');
					$dimention_cm = pods_field('product', $product_id, 'dimension-cm');
					
					if (!empty($dimention_in)) {
						echo '<span class="dimension-cm">' . esc_attr($dimention_in[0]) . '</span> <br>';
					}
				
					if (!empty($dimention_cm)) {
						echo '<span class="dimension-cm">' . esc_attr($dimention_cm[0]) . '</span>';
					}
					?>
					...
					do_action( 'yith_wcwl_table_before_product_name', $item, $wishlist );
					$product_id = $item['product_id'];
					$pdf_url = pods_field('product', $product_id, 'sheet');

					if (!empty($pdf_url)) {
						?>
						<a href="<?php echo esc_url($pdf_url['guid']); ?>" target="_blank">
							<img loading="lazy" width="20" height="20" src="https://emmanuellesimon.com/wp-content/uploads/2024/01/Group-60.svg" 
								 class="attachment-thumbnail size-thumbnail wp-image-33294" 
								 alt="télécharger la fiche technique">
						</a>
						<?php
					}
					?>
					</td>
					  <td class="product-request-price">
						<?php
							do_action( 'yith_wcwl_table_before_product_name', $item, $wishlist );
							$product_id = $item['product_id'];
							$collection_name = pods_field('product', $product_id, 'collection_name');
							$reference = pods_field('product', $product_id, 'ref');
							
							$price_title = esc_html( pll__( 'Demande de prix') );
							$product = wc_get_product($product_id);
							$product_tags = wp_get_post_terms($product_id, 'esa_product_tags');
							
							$flag_enquete_tag = false;
							foreach ($product_tags as $tag) {
								if ($tag->slug === 'enquete') {
									$flag_enquete_tag = true;
									break;
								}
							}
							if ($flag_enquete_tag || empty($product->get_price())) { ?>
								<a href="#" class="request-price-btn-wishlist" data-product-name="<?php echo wp_kses_post( apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ); ?>" data-category-name="<?php echo $collection_name[0]; ?>" data-product-ref="<?php echo $reference[0]; ?>"><?php echo $price_title;?></a>
							<?php
							} else {
